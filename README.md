...
| 劣势 | 说明 |
|---|---|
| **违反 ToS，账号有风险** | 逆向私有协议违反 DeepSeek 服务条款，封号风险永远不为零。用专用小号，别用主力号；风险自担 |
| **没有 SLA，上游一变就失效** | 依赖网页私有协议：DeepSeek 改版、PoW 算法升级、接口调整都会让桥失效；协议来源 deepseek-pp 已归档，没人保证跟进修复 |
...
| **模型选择零灵活性** | 只能用网页版当前提供的模型（现为 deepseek-v4.1-flash），选不了官方 API 的其他型号；上下文长度以网页版为准 |
| **单机单用户设计** | 只绑定 127.0.0.1，凭据是明文本地文件——按"自己机器自己用"设计，不要当共享服务暴露出去 |
