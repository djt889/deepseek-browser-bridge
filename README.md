...
| 劣势 | 说明 |
|---|---|
| **延迟与吞吐低于官方 API** | 请求过一层真实浏览器（CDP 往返 + 页面 fetch），首 token 延迟更高。**并行能力受账号数限制**：单账号实测 3 路真并行稳定（6 请求 100% 成功、2.9x 加速），6 路会触发风控产空流（实测仅 2/6 成功）。要更高并行请加账号（并行度 = 账号数 × 3） |
| **官方风控无法技术绕过** | 短时间高频会触发软限流，恢复可达数小时且期间只能干等；继续猛打会升级到强制下线。桥会主动识别：完全空流（无正文无思考）即判定风控压力，**该账号自动退避到 1 路并持续 10 分钟**，避免升级为真冷却。只能靠节流预防，没有事后补救手段 |
...
**并行模型**：真实并行度 = `存活账号数 × DQ_PER_ACCOUNT_CONCURRENCY`（受 `DQ_CONCURRENCY` 封顶）。请求**不再等待上一条完成**——同一账号内最多 N 个同时在生成，只把**启动时刻**错开，避免同毫秒的整批爆发。

| 变量 | 默认 | 说明 |
|---|---|---|
| `DQ_CONCURRENCY` | 6 | 全局并行上限（整个桥同时运行的任务数）|
| `DQ_PER_ACCOUNT_CONCURRENCY` | 3 | **每账号同时生成数**。单账号实测：3 路成功率 100%；**6 路会触发风控**，出现大量空流（实测仅 2/6 成功），故默认保守为 3。要更高的真并行请加账号 |
| `DQ_START_STAGGER_MS` | 1500 | 同一账号两次启动的最小间隔（仅错开启动，不等待完成）|
...
| `DQ_MIN_GAP_MS` | 0 | **旧版串行节流**（完成后再等 N 毫秒），默认关闭。设 >0 可退回串行模式 |
...
| `DQ_THINK` | 1 | 思考全局默认（模型名可按请求覆盖）。**注意：实测思考开启显著提升工具调用遵循率（100% vs 67%），建议保持开启** |
...
- **并发实测（2026-09-21）**：单账号 3 路真并行下 6 个并发请求 **100% 成功、2.9x 加速**（总墙钟等于最慢单个而非耗时之和）；提到 6 路则出现大量完全空流（**仅 2/6 成功**），说明单账号安全上限约 3 路。空流会被识别为风控压力并自动退避。
